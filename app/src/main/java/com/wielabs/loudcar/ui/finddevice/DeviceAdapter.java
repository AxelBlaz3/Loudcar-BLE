...
public class DeviceAdapter extends ListAdapter<BluetoothDevice, DeviceAdapter.DeviceViewHolder> {
    ...
    static class DeviceDiffUtil extends DiffUtil.ItemCallback<BluetoothDevice> {

        @Override
        public boolean areItemsTheSame(@NonNull BluetoothDevice oldItem, @NonNull BluetoothDevice newItem) {
            return oldItem.getAddress().equalsIgnoreCase(newItem.getAddress());
        }
    ...
    protected DeviceAdapter(@NonNull DiffUtil.ItemCallback diffCallback, DeviceAdapterListener listener) {
        super(diffCallback);
